...
class BatchSymbolicExecutor:
    ...
    def run_symbolic_execution(self, binary_path, output_dir=None):
        ...
        try:
            ...
            # 运行符号执行 - 在当前目录执行，而不是在output_dir
            result = subprocess.run(
                cmd,
                cwd=os.getcwd(),  # 使用当前工作目录
                capture_output=True,
                text=True,
                timeout=self.timeout + 30  # 额外30秒缓冲时间
            )
            ...
            if result.returncode == 0:
                ...
                analysis_result['error_output'] = result.stderr[:500]  # 只保存前500字符的错误信息
    ...
    def preview_analysis(self):
        ...
        if total_estimated_time > 3600:  # 超过1小时
            print(f"\n⚠️  预估时间较长，建议后台运行:")
