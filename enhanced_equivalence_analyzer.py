...
class LayeredEquivalenceResult:
    """分层等价性检查结果"""
    level1_control_flow: str  # equivalent, not_equivalent, unknown
...
class ConstraintClassifier:
    ...
    def __init__(self):
        ...
        self.control_flow_patterns = [
            r'bvslt.*scanf_0',  # 循环边界条件
            r'bvsge.*scanf_0',  # 循环边界条件  
            r'bvuge.*scanf_0',  # 输入范围检查
            r'bvule.*scanf_0',  # 输入范围检查
            r'distinct.*scanf_0', # 分支条件
        ]
        
        # 内存访问模式
        self.memory_access_patterns = [
            r'distinct.*bv\d+.*64',  # 内存地址不等性
            r'bvule.*bv\d{7,}',      # 内存边界检查
            r'bvadd.*bv\d{7,}',      # 地址计算
            r'bvshl.*bv\d+.*64',     # 地址偏移计算
        ]
        
        # 数据变换模式  
        self.data_transformation_patterns = [
            r'bvadd.*extract',  # 数据加法运算
            r'bvsub.*extract',  # 数据减法运算
            r'bvmul.*extract',  # 数据乘法运算
            r'bvand.*extract',  # 位运算
            r'bvor.*extract',   # 位运算
            r'select.*store',   # 内存读写操作
        ]
...
class LayeredEquivalenceChecker:
    ...
    def calculate_confidence_score(self, level1_result: str, level2_result: str, 
        ...
        scores = [
            score_map.get(level1_result, 0.3) * 0.3,  # 控制流权重30%
            score_map.get(level2_result, 0.3) * 0.4,  # 内存访问权重40%
            score_map.get(level3_result, 0.3) * 0.3   # 数据变换权重30%
        ]
