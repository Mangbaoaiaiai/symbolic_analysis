...
class EnhancedMockTSVCAnalyzer:
        ...
        self.benchmark_patterns = {
            ...
            's121': {
                ...
                'optimization_effects': {
                    ...
                    'O3': ['dependency_block']  # O3无法优化由于依赖
                }
            },
            's1221': {
    ...
    def generate_realistic_constraints(self, benchmark_name: str, optimization: str, path_index: int) -> Dict:
        ...
        variables = ['i', 'count']
        constraints = [
            f"(assert (= count #x00000001))",  # count = 1
        ]
        ...
        if benchmark_name == 's000':
            ...
            constraints.extend([
                f"(assert (= a (bvadd b #x00000001)))",  # a = b + 1
                f"(assert (bvule i #x00000008))"  # i <= 8
            ])
            ...
            constraints.extend([
                f"(assert (= a (bvadd b #x00000001)))",  # a = b + 1
                f"(assert (bvuge i #x00000000))",  # i >= 0 (reverse)
                f"(assert (bvule i #x00000008))"   # i <= 8
            ])
            ...
            constraints.extend([
                f"(assert (= a_curr (bvadd a_next b)))",  # a[i] = a[i+1] + b[i]
                f"(assert (bvult i #x00000007))",  # i < 7 (dependency limit)
                f"(assert (distinct a_curr a_next))"  # 数据依赖约束
            ])
            
        elif benchmark_name == 's1221':
            variables.extend(['a_curr', 'a_prev4', 'b'])
            constraints.extend([
                f"(assert (= a_curr (bvadd a_prev4 b)))",  # a[i] = a[i-4] + b[i]
                f"(assert (bvuge i #x00000004))",  # i >= 4
                f"(assert (bvule i #x00000008))"   # i <= 8
            ])
            ...
            constraints.extend([
                f"(assert (= a1 (bvadd b e)))",     # a[i+1] = b[i] + e[i]
                f"(assert (= a2 (bvadd b c)))",     # a[i] = b[i] + c[i]  
                f"(assert (bvult i #x00000007))",  # i < 7
                f"(assert (distinct a1 a2))"       # 不同数组元素
            ])
            
        elif benchmark_name == 'vpv':
            variables.extend(['a', 'b', 'c'])
            constraints.extend([
                f"(assert (= a (bvmul b c)))",      # a[i] = b[i] * c[i]
                f"(assert (bvule i #x00000008))"   # i <= 8
            ])
        ...
        for opt_effect in opt_effects:
            if opt_effect == 'loop_unroll':
                ...
                constraints.append(f"(assert (= unroll_factor #x00000004))")  # 展开4次
                
            elif opt_effect == 'vectorization':
                variables.extend(['vector_width', 'simd_lanes'])
                constraints.extend([
                    f"(assert (= vector_width #x00000004))",  # 4-wide SIMD
                    f"(assert (= simd_lanes #x00000004))"
                ])
                
            elif opt_effect == 'prefetch':
                variables.append('prefetch_distance')
                constraints.append(f"(assert (= prefetch_distance #x00000010))")  # 预取16字节
                ...
                constraints.append(f"(assert (= optimization_blocked #x00000001))")  # 优化受阻
