...
class ImprovedJavaToCConverter:
    def __init__(self, benchmarks_root):
        self.benchmarks_root = Path(benchmarks_root)
        self.converted_count = 0
        self.failed_count = 0
        
    def clean_java_code(self, content):
        """清理Java代码，移除/转换Java特有语法"""
                             
        content = re.sub(r'\bpublic\s+', '', content)
        content = re.sub(r'\bprivate\s+', '', content)
        content = re.sub(r'\bprotected\s+', '', content)
        
                     
        content = content.replace('boolean', 'int')
        content = content.replace('true', '1')
        content = content.replace('false', '0')
        
                
        content = re.sub(r'int\s*\[\s*\]', 'int*', content)
        content = re.sub(r'double\s*\[\s*\]', 'double*', content)
        content = re.sub(r'float\s*\[\s*\]', 'float*', content)
        
                   
        content = re.sub(r'new\s+int\s*\[\s*(\d+)\s*\]', r'malloc(\1 * sizeof(int))', content)
        content = re.sub(r'new\s+double\s*\[\s*(\d+)\s*\]', r'malloc(\1 * sizeof(double))', content)
    ...
    def needs_stdlib(self, content):
        """检查是否需要stdlib.h"""
        return 'malloc' in content or 'free' in content
    ...
    def extract_snippet_function(self, java_content):
        """改进的snippet函数提取"""
                        
        pattern = r'(static\s+\w+\s+snippet\s*\([^)]*\)\s*\{)'
        ...
        sig_pattern = r'static\s+(\w+)\s+snippet\s*\(([^)]*)\)'
        sig_match = re.search(sig_pattern, func_signature)
        ...
        brace_count = 0
        end_pos = len(java_content)           
        
        for i, char in enumerate(java_content[start_pos:], start_pos):
            if char == '{':
                brace_count += 1
            elif char == '}':
                brace_count -= 1
        ...
        function_body = java_content[start_pos:end_pos]
        
        return return_type, params, function_body
    ...
    def convert_java_to_c(self, java_file):
        ...
        try:
            ...
            return_type, params, function_body = self.extract_snippet_function(java_content)
            
            if return_type is None or params is None or function_body is None:
                print(f"无法提取snippet函数: {java_file}")
                return False
            
                     
            function_body = self.clean_java_code(function_body)
            ...
            c_code = self.generate_c_code(
                ...
                params, self.needs_math_h(function_body), self.needs_stdlib(function_body)
            )
    ...
    def generate_c_code(self, return_type, params_str, function_body, params, needs_math, needs_stdlib):
        """生成完整的C代码"""
        ...
        if not params:
            main_body = f'''    {return_type} result = snippet();
    printf("Result: %f\\n", (double)result);
    ...
    def convert_simple_tests_only(self):
        """只转换简单的测试（排除复杂的TCAS等）"""
        print("开始转换简单测试...")
        
                     
        simple_patterns = [
            'Airy', 'ModDiff', 'Bess', 'Ell', 'gam', 'Ran', 
            'power', 'caldat', 'dart', 'sine'
        ]
        
        test_dirs = []
        for pattern in simple_patterns:
            for java_file in self.benchmarks_root.rglob(f'{pattern}/**/oldV.java'):
                test_dirs.append(java_file.parent)
            for java_file in self.benchmarks_root.rglob(f'{pattern}/**/newV.java'):
                test_dirs.append(java_file.parent)
        ...
        print(f"找到 {len(test_dirs)} 个简单测试目录")
...
if __name__ == "__main__":
    converter = ImprovedJavaToCConverter("/root/ardiff/symbolic_analysis/benchmarks")
    converter.convert_simple_tests_only() 