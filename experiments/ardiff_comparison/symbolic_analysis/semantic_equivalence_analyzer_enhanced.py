...
class EnhancedConstraintChecker:
    ...
    def check_constraint_and_output_equivalence(self, path1_info, path2_info):
        ...
        if constraint_result == "equivalent":
            ...
            print(f"      ❌ 约束不等价: {constraint_result} (耗时: {constraint_time:.3f}s)")
        ...
        if output_equivalent:
            print(f"      ✓ 程序输出相同: '{output1}' (耗时: {output_time:.3f}s)")
        else:
            print(f"      ❌ 程序输出不同: '{output1}' vs '{output2}' (耗时: {output_time:.3f}s)")
...
class EnhancedPathAnalyzer:
    ...
    def find_equivalent_paths_enhanced(self, paths1, paths2):
        ...
        print(f"\n开始增强的等价性验证 ({total_comparisons} 对比较):")
        ...
        for i, path1 in enumerate(paths1):
            ...
            for j, path2 in enumerate(paths2):
                ...
                if equivalence_result['overall_equivalent']:
                    ...
                    print(f"    ✅ 完全等价! 约束✓ 输出✓ 耗时: {pair_time:.3f}s")
                    path1_matched = True
                    break           
                    ...
                    print(f"    ⚠️  疑似不等价（约束）! 约束❌ 输出✓ 耗时: {pair_time:.3f}s")
                    ...
                    print(f"    ⚠️  疑似不等价（输出）! 约束✓ 输出❌ 耗时: {pair_time:.3f}s")
                    
                else:
                                       
                    results['non_equivalent_pairs'].append(pair_info)
                    print(f"    ❌ 不等价! 约束❌ 输出❌ 耗时: {pair_time:.3f}s")
        ...
        print(f"  疑似不等价（约束）: {len(results['suspicious_constraint_pairs'])}")
        print(f"  疑似不等价（输出）: {len(results['suspicious_output_pairs'])}")
    ...
    def generate_comprehensive_report(self, results, output_file="enhanced_equivalence_report.txt"):
        ...
        with open(output_file, "w", encoding='utf-8') as f:
            ...
            f.write(f"疑似不等价（约束）: {len(results['suspicious_constraint_pairs'])}\n")
            f.write(f"疑似不等价（输出）: {len(results['suspicious_output_pairs'])}\n")
            ...
            if results['equivalent_pairs']:
                ...
                for idx, pair in enumerate(results['equivalent_pairs'], 1):
                    ...
                    f.write(f"   程序输出: '{equiv_result['path1_output']}' = '{equiv_result['path2_output']}'\n")
                    f.write(f"   变量映射: {equiv_result['variable_mapping']}\n\n")
            
                         
            if results['suspicious_constraint_pairs']:
                f.write("⚠️  疑似不等价（约束）路径对:\n")
                f.write("-" * 30 + "\n")
                for idx, pair in enumerate(results['suspicious_constraint_pairs'], 1):
                    ...
                    f.write(f"     - 程序输出相同: {'✅' if equiv_result['output_equivalent'] else '❌'}\n")
                    f.write(f"   程序输出: '{equiv_result['path1_output']}' = '{equiv_result['path2_output']}'\n")
                    f.write(f"   比较耗时: {pair['comparison_time']:.3f} 秒\n\n")
            
                         
            if results['suspicious_output_pairs']:
                f.write("⚠️  疑似不等价（输出）路径对:\n")
                f.write("-" * 30 + "\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='增强的程序等价性分析器：支持约束和输出的综合判断')
    ...
    print("  2️⃣  程序输出等价性检查")
    print("  3️⃣  数组初始状态一致性检查")
    print("  4️⃣  数组最终状态一致性检查")
    print("=" * 60)
