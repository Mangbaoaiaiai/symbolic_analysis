...
def find_path_matches_fixed(paths1, paths2):
    ...
    matches = {
        'exact_variable_matches': [],    # 变量值完全相同
        'exact_output_matches': [],      # 程序输出完全相同
        'similar_constraint_matches': [], # 约束结构相似
        'approximate_matches': [],       # 综合距离较小
        'no_matches': []                # 无匹配
    }
    ...
    for i, path1 in enumerate(paths1):
        ...
        for j, path2 in enumerate(paths2):
            ...
            # 3. 检查约束结构相似
            elif distance['constraint_count'] <= 1:  # 约束数量差异不超过1
        ...
        if best_match_type == 'exact_output':
            ...
            used_paths2.add(best_match)
        elif best_match_type == 'approximate' and best_distance < 200:  # 阈值
