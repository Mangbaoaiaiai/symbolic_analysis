...
def analyze_program_semantics():
    ...
    # 测试数据
    test_count = 1  # count=1，满足约束条件
    a_init = [i % 100 for i in range(16)]  # 初始化a数组
    b_init = [(i * 2) % 100 for i in range(16)]  # 初始化b数组
    ...
    if differences:
        ...
        for i, val_s000, val_s121 in differences[:5]:  # 只显示前5个差异
