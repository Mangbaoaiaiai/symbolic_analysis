        ...
        for line in lines:
            ...
            if line.startswith('; 数组初始值:'):
                try:
                    ...
                    array_initial = eval(array_str)  # 安全性：生产环境中应使用ast.literal_eval
                except:
                    pass
            ...
            # 解析数组最终值
                ...
                try:
                    array_str = line.split(':', 1)[1].strip()
                    array_final = eval(array_str)  # 安全性：生产环境中应使用ast.literal_eval
                except:
                    pass
...
class EnhancedPathAnalyzer:
    ...
    def __init__(self):
        ...
        self.symbolic_execution_time = 0.0  # 假设从外部获取
    ...
    def find_equivalent_paths_three_step(self, paths1, paths2):
        ...
        results = {
            'equivalent_pairs': [],
            'partial_equivalent_pairs': [],  # 部分等价的路径对
