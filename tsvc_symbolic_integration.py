...
class TSVCSymbolicIntegrator:
    ...
    def generate_execution_paths(self, binary_path, benchmark_name, num_paths=50):
        ...
        # 模拟路径生成（您需要替换为实际的路径生成工具）
        # 这里假设您有一个工具可以从二进制文件生成路径约束
        ...
        try:
            ...
            for i in range(min(num_paths, 20)):  # 限制路径数量避免过长运行
    ...
    def generate_mock_smt_constraints(self, benchmark_name, path_index):
        ...
        if 's000' in benchmark_name:
            ...
            constraints = [
                ...
                f"(bvule scanf_{path_index}_i #x00000080)"  # i < 128
            ]
        elif 's121' in benchmark_name:
            # s121: a[i] = a[i+1] + b[i] - 需要额外声明a_next
            ...
            constraints = [
                ...
                f"(bvult scanf_{path_index}_i #x0000007f)"  # i < 127
            ]
        else:
            # 通用约束 - 使用已声明的变量
            constraints = [
